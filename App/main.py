...
def read_detection(detection_id: int, db: Session = Depends(get_db)):
    ...
    return db_detection
# PUT endpoint to update detection
@app.put("/detections/{detection_id}", response_model=schemas.Detection)
def update_detection(detection_id: int, detection: schemas.DetectionUpdate, db: Session = Depends(get_db)):
    db_detection = crud.update_detection(db=db, detection_id=detection_id, detection_update=detection)
